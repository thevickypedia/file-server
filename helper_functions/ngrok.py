...
def restart() -> None:
    ...
    """
    if pid := checker():
        logger.info(f'{pid} will be terminated to close the active session.')
        system(f'kill -9 {pid} >/dev/null 2>&1')  # redirects stderr output to stdout
...
def tunnel() -> None:
    ...
    if path.isfile('ngrok.yml'):
        get_default().config_path = 'ngrok.yml'

    if auth_token := environ.get('ngrok_auth'):
